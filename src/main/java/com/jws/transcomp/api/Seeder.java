...
public class Seeder implements ApplicationRunner {
    ...
    private void companySeed() {
        ...
        List<Company> companies = Arrays.asList(
                new Company("Ekont", new HashSet<>()),
                new Company("Speedy", new HashSet<>()),
                new Company("Union Ivkoni", new HashSet<>())
        );
    ...
    private void vehicleSeed() {
        if (vehicleService.any()) {
            return;
        }

        List<String> vehicleMakes = Arrays.asList("Ford", "Toyota", "Mercedes-Benz", "Renault", "Fiat");
        ...
        for (int i = 0; i < 100; i++) {
            ...
            Company comp = companyService.findAll().get(rGenerator.nextInt((int) companyService.count()));
    ...
    private void clientSeed() {
        ...
        for (int i = 0; i < 10; i++) {
            ...
            comps.add(allCompanies.get(rGenerator.nextInt(allCompanies.size())));
            comps.add(allCompanies.get(rGenerator.nextInt(allCompanies.size())));
